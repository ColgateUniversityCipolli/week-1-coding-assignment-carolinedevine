...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \textbf{Solution:}
<<Plot4, eval=FALSE, size="scriptsize", warning=FALSE, message=FALSE>>=
set.seed(1)
canvas_smoke(colors = colorPalette("random", 1024), init = 1, shape = c("bursts", "clouds"), resolution = 200)
@
%Code to insert a figure [H]ere
\begin{figure}[H]
\begin{center}
...
\textbf{Solution:}
 ...
 <<eval=TRUE, size="scriptsize", warning=FALSE, message=FALSE>>=
  algebra.vector <- 2^(numeric.vector + 1) + 2^(numeric.vector - 1)
  algebra.vector
  @
...
\textbf{Solution:}
  ...
  <<eval=TRUE, size="scriptsize", warning=FALSE, message=FALSE>>=
  result.forty <- which(algebra.vector == 40)
  result.forty
  @
...
\textbf{Solution:}
  ...
  <<eval=TRUE, size="scriptsize", warning=FALSE, message=FALSE>>=
  x.solution <- numeric.vector[result.forty]
  x.solution
  @
