...
\begin{alltt}
\hlkwd{set.seed}\hldef{(}\hlnum{1}\hldef{)}
\hlkwd{canvas_smoke}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"random"}\hldef{,} \hlnum{1024}\hldef{),} \hlkwc{init} \hldef{=} \hlnum{1}\hldef{,} \hlkwc{shape} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"bursts"}\hldef{,} \hlsng{"clouds"}\hldef{),} \hlkwc{resolution} \hldef{=} \hlnum{200}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
%Code to insert a figure [H]ere
...
% Numeric Vector with integers 0 to 10 inclusive %
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{numeric.vector} \hlkwb{<-} \hlkwd{c}\hldef{(}\hlnum{0}\hlopt{:}\hlnum{10}\hldef{)}
\hldef{numeric.vector}
\end{alltt}
\begin{verbatim}
...
\textbf{Solution:}
 ...
 % Computing for each value in numeric.vector
 
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
 \hldef{algebra.vector} \hlkwb{<-} \hlnum{2}\hlopt{^}\hldef{(numeric.vector} \hlopt{+} \hlnum{1}\hldef{)} \hlopt{+} \hlnum{2}\hlopt{^}\hldef{(numeric.vector} \hlopt{-} \hlnum{1}\hldef{)}
 \hldef{algebra.vector}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{result.forty} \hlkwb{<-} \hlkwd{which}\hldef{(algebra.vector} \hlopt{==} \hlnum{40}\hldef{)}
\hldef{result.forty}
\end{alltt}
\begin{verbatim}
## [1] 5
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{x.solution} \hlkwb{<-} \hldef{numeric.vector[result.forty]}
\hldef{x.solution}
\end{alltt}
\begin{verbatim}
## [1] 4
...
The approach we used above will not work due to the integer vector that we are using. Since the solution to this new potential problem is a floating point number, the vector would not loop through the correct solution because it is only running through 0, 1, ... , 10 and not numbers like 1.5 or 2.012. 
